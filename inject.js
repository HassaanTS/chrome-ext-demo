const findColumnIndex = (headers, key) => {
    for (let i = 0; i < headers.length; i++) {
        if (headers[i].firstElementChild &&
...
const findValues = (rows, columnIndex, isNum) => {
    ...
    for (let i = 0; i < rows.length; i++) {
        if (rows[i].children[columnIndex] && 
            rows[i].children[columnIndex].firstElementChild && 
            rows[i].children[columnIndex].firstElementChild.firstChild) 
            {
                if(isNum){
                    var thing = parseFloat(rows[i].children[columnIndex].firstElementChild.firstChild.innerText);
                    if (!isNaN(thing)) {
                        values.push(thing);
                    }
                }
                else{
                    value = rows[i].children[columnIndex].firstElementChild.firstChild.innerText
...
const findBubbleValues = (rows, columnIndex) => {
    ...
    for (let i = 0; i < rows.length; i++) {
        if (rows[i].children[columnIndex] && 
            ...
            rows[i].children[columnIndex].children[0].children[0].children[0].children[0] && 
            rows[i].children[columnIndex].children[0].children[0].children[0].children[0].firstChild) 
            {
                value = rows[i].children[columnIndex].children[0].children[0].children[0].children[0].firstChild.innerText
                values.push(value);
            }
...
(
    function() {
        ...
        sizeStats = findValues(rows, sizeIndex, true).reduce((partialSum, elem) => partialSum + elem, 0);
